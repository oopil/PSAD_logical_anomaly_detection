...
from components import GaussianBlur2d
...
class AnomalyMapGenerator(nn.Module):
    ...
    def compute_anomaly_score(patch_scores: torch.Tensor) -> torch.Tensor:
        ...
        # if np.isnan(score.detach().cpu().item()):
        #     pdb.set_trace()
        #     print('')
    ...
    def forward(self, **kwargs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        anomaly_map, anomaly_map_ = self.compute_anomaly_map(patch_scores, feature_map_shape)
        anomaly_score = self.compute_anomaly_score(patch_scores)

        return anomaly_map, anomaly_score, anomaly_map_
